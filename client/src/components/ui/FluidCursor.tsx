...
const FluidCursor = () => {
    ...
    `,
        ...
        let copyProgram: any = new Program(baseVertexShader!, copyShader!);
        let clearProgram: any = new Program(baseVertexShader!, clearShader!);
        let colorProgram: any = new Program(baseVertexShader!, colorShader!);
        let checkerboardProgram: any = new Program(baseVertexShader!, checkerboardShader!);
        let blurProgram: any = new Program(blurVertexShader!, blurShader!);
        let splatProgram: any = new Program(baseVertexShader!, splatShader!);
        let advectionProgram: any = new Program(baseVertexShader!, advectionShader!);
        let divergenceProgram: any = new Program(baseVertexShader!, divergenceShader!);
        let curlProgram: any = new Program(baseVertexShader!, curlShader!);
        let vorticityProgram: any = new Program(baseVertexShader!, vorticityShader!);
        let pressureProgram: any = new Program(baseVertexShader!, pressureShader!);
        let gradienSubtractProgram: any = new Program(baseVertexShader!, gradientSubtractShader!);

        let displayMaterial: any = new Material(baseVertexShader, displayShaderSource);
        ...
        function createDoubleFBO(w: any, h: any, internalFormat: any, format: any, type: any, param: any) {
            ...
            return {
                width: w,
                height: h,
                texelSizeX: fbo1.width ? 1.0 / fbo1.width : 0.0,
                texelSizeY: fbo1.height ? 1.0 / fbo1.height : 0.0,
                read: fbo1,
                write: fbo2,
                swap() {
                    let temp = this.read;
    ...
    return (
        <div className="fixed top-0 left-0 z-50 h-screen w-screen pointer-events-none">
            <canvas
                ref={canvasRef}
